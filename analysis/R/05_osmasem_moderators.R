...
# ==============================================================================
# Purpose: Test continuous moderators using OSMASEM (Jak & Cheung, 2020)
...
# Hofstede individualism score
if ("individualism" %in% colnames(metadata)) {
  osmasem_data$indiv <- metadata$individualism
  message(sprintf("  Individualism: M = %.1f, SD = %.1f, range %d-%d",
                  mean(osmasem_data$indiv, na.rm = TRUE),
                  sd(osmasem_data$indiv, na.rm = TRUE),
                  min(osmasem_data$indiv, na.rm = TRUE),
                  max(osmasem_data$indiv, na.rm = TRUE)))
} else {
  warning("  'individualism' column not found in metadata. Skipping individualism moderator.")
  osmasem_data$indiv <- NA
}
...
# ==============================================================================
# 7. MODERATOR: HOFSTEDE INDIVIDUALISM
# ==============================================================================
message("\n--- Testing Moderator: Hofstede Individualism ---")

indiv_results <- list()

if (!all(is.na(osmasem_data$indiv)) && !is.null(osmasem_baseline)) {
  tryCatch({
    Ax_indiv <- matrix(0, nrow = N_CONSTRUCTS, ncol = N_CONSTRUCTS,
                       dimnames = list(CONSTRUCTS, CONSTRUCTS))
    ...
    for (path in key_paths) {
      ...
      iv <- parts[1]
      dv <- parts[2]
      Ax_indiv[dv, iv] <- paste0("0*indiv_", path)
    }

    Ax_indiv_mx <- as.mxMatrix(Ax_indiv, name = "Ax")

    osmasem_indiv <- osmasem(
      model.name = "Individualism_Moderation",
      RAM = list(A = Amatrix, S = Smatrix, F = Fmatrix),
      Ax = Ax_indiv_mx,
      data = cor_matrices,
      n = sample_sizes,
    ...
    summary_indiv <- summary(osmasem_indiv)
    lr_indiv <- anova(osmasem_indiv, osmasem_baseline)

    indiv_results$model <- osmasem_indiv
    indiv_results$summary <- summary_indiv
    indiv_results$lr_test <- lr_indiv
    indiv_results$coefficients <- coef(osmasem_indiv)

    message("  Individualism moderation model fitted")
    message(sprintf("  LR chi-sq = %.3f, df = %d, p = %.4f",
                    lr_indiv$diffLL[2], lr_indiv$diffdf[2], lr_indiv$p[2]))

    mod_coefs <- coef(osmasem_indiv)
    indiv_mod_idx <- grep("indiv_", names(mod_coefs))
    if (length(indiv_mod_idx) > 0) {
      message("\n  Individualism moderation coefficients:")
      for (idx in indiv_mod_idx) {
        message(sprintf("    %s = %.4f", names(mod_coefs)[idx], mod_coefs[idx]))
      }
    }

  }, error = function(e) {
    message(sprintf("  WARNING: Individualism moderation failed: %s", e$message))
    indiv_results$error <<- e$message
  })
} else {
  message("  Skipped: individualism data unavailable or baseline model failed")
  indiv_results$error <- "Data unavailable"
}
...
osmasem_results <- list(
  ...
  individualism_moderation = indiv_results,
