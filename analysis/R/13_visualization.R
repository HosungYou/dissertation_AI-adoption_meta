...
# ==============================================================================
# Purpose: Generate all publication-quality figures for the dissertation
...
message("\n", strrep("=", 70))
message("13: COMPREHENSIVE VISUALIZATION SUITE")
message(strrep("=", 70))
  ...
  tryCatch({
    ...
    if (length(plot_list) > 0) {
      combined <- wrap_plots(plot_list, ncol = 2) +
        plot_annotation(
          title = "Bayesian Prior-to-Posterior Shift: General IT (Sabherwal) vs AI Context",
...
if (!is.null(network) && !is.null(network$omega_matrix)) {
  ...
  save_base_plot(function() {
    qgraph(
      ...
      negCol = "#B2182B",
      negDashed = TRUE,
      title = "AI Adoption Meta-Analytic Network (MAGNA)",
      legend = TRUE,
      legend.cex = 0.55,
      mar = c(5, 5, 6, 5)
    )
...
if (!is.null(tssem)) {
  ...
  if (length(model_names) > 0) {
    ...
    p_fit <- ggplot(fit_data, aes(x = Model, y = Value, fill = Model)) +
      ...
      labs(
        title = "Model Fit Comparison: Three Competing Structural Models",
        subtitle = "Red dashed line = threshold for good fit",
        x = "", y = "Fit Index Value"
      ) +
...
if (!is.null(het) && !is.null(het$I2_matrix)) {
  ...
  p_heatmap <- ggplot(I2_long, aes(x = Var1, y = Var2, fill = I2)) +
    ...
    labs(
      title = expression("Heterogeneity Heatmap: " * I^2 * " (%) for 66 Pairwise Correlations"),
      subtitle = "Blue = low heterogeneity, Red = high heterogeneity",
      x = "", y = ""
    ) +
    theme_minimal(base_size = 12) +
    theme(
...
if (!is.null(temporal) && !is.null(temporal$comparison_table)) {
  ...
  p_temporal <- ggplot(plot_data, aes(x = Path, y = Estimate, fill = Era)) +
    ...
    labs(
      title = "Pre vs Post ChatGPT: Path Coefficient Comparison",
...
if (!is.null(stage1) && !is.null(stage1$pooled_matrix)) {
  ...
  p_cormat <- ggplot(cor_long, aes(x = Var1, y = Var2, fill = r_display)) +
    ...
    labs(
      title = "Pooled Meta-Analytic Correlation Matrix",
  ...
  save_base_plot(function() {
    corrplot(
      ...
      title = sprintf("Pooled Correlation Matrix (k=%d, N=%d)",
...
if (!is.null(pubbias) && !is.null(pubbias$bias_summary_table)) {
  ...
  p_bias <- ggplot(comp_long, aes(x = reorder(Pair, Estimate), y = Estimate,
    ...
    coord_flip() +
    labs(
      title = "Publication Bias Assessment: Original vs Adjusted Effect Sizes",
...
if (!is.null(sensitivity) && !is.null(sensitivity$comparison_table)) {
  ...
  p_sens <- ggplot(comp, aes(x = Full_Est, y = Ronly_Est)) +
    ...
    labs(
      title = "Sensitivity: Full Dataset vs R-only Subset",
      subtitle = sprintf("r = %.3f | max |diff| = %.4f | Decision: %s",
