...
# ==============================================================================
# Purpose: Compare AI adoption structural relationships before vs after ChatGPT
...
message("\n", strrep("=", 70))
message("06: TEMPORAL ANALYSIS — PRE/POST ChatGPT COMPARISON")
message(strrep("=", 70))
...
if (!is.null(comparison_table)) {
  ...
  p_temporal <- ggplot(plot_data, aes(x = Path, y = Estimate, fill = Era)) +
    ...
    labs(
      title = "Path Coefficient Comparison: Pre vs Post ChatGPT",
  ...
  p_diff <- ggplot(comparison_table, aes(x = reorder(Path, abs(Diff)), y = Diff)) +
    ...
    coord_flip() +
    labs(
      title = "Temporal Change in Path Coefficients (Post - Pre ChatGPT)",
