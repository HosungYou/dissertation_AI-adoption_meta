...
This guide explains Meta-Analytic Structural Equation Modeling (MASEM) and its application to AI adoption research. MASEM synthesizes correlation matrices across studies to test structural models, combining the strengths of meta-analysis and SEM.
...
- Test continuous moderators (culture, year)
- Subgroup TSSEM for categorical moderators (AI type, industry)
...
**Example: Generative AI vs. Predictive AI**

```r
# Split data by AI type
generative_studies <- studies[studies$ai_type == "Generative", ]
predictive_studies <- studies[studies$ai_type == "Predictive", ]

# Run Stage 1 separately
stage1_gen <- tssem1(Cov = cor_matrices_gen, n = n_gen, method = "REM")
stage1_pred <- tssem1(Cov = cor_matrices_pred, n = n_pred, method = "REM")

# Fit same model on both
model_gen <- tssem2(stage1_gen, Amatrix=A, Smatrix=S, Fmatrix=F)
model_pred <- tssem2(stage1_pred, Amatrix=A, Smatrix=S, Fmatrix=F)

# Compare path coefficients
coef_gen <- coef(model_gen)
coef_pred <- coef(model_pred)
...
- Example: β(PE→BI) = .35 + .02×(year-2020)
  - Interpretation: PE→BI path increases by .02 per year
...
|-------------|---|---|---|--------|---|----|----|
| PE-BI | 145 | 38,492 | .52 | [.49, .55] | 485.3*** | 68% | .012 |
| EE-BI | 140 | 36,218 | .48 | [.45, .51] | 412.8*** | 64% | .010 |
| TRU-BI | 78 | 19,856 | .58 | [.54, .62] | 298.5*** | 72% | .018 |
