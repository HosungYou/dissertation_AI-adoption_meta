...
This protocol describes Meta-Analytic Gaussian Network Aggregation (MAGNA) as a complementary approach to MASEM. Network analysis provides an exploratory, data-driven perspective on construct relationships without imposing directional assumptions, allowing triangulation with MASEM findings.
...
**Question:** Does the network structure differ between generative AI and predictive AI?

**Hypothesis:**
- Generative AI: TRU—BI edge stronger (trust matters more for black-box generative systems)
- Predictive AI: PE—BI edge stronger (performance matters more for decision-support)
...
# Split data by AI type
cor_generative <- cor_matrices[ai_type == "Generative"]
cor_predictive <- cor_matrices[ai_type == "Predictive"]

# Pool correlations separately
stage1_gen <- tssem1(Cov = cor_generative, n = n_gen)
stage1_pred <- tssem1(Cov = cor_predictive, n = n_pred)

pooled_cor_gen <- vec2symMat(coef(stage1_gen), diag = FALSE)
pooled_cor_pred <- vec2symMat(coef(stage1_pred), diag = FALSE)

# Estimate networks
net_gen <- estimateNetwork(pooled_cor_gen, default = "EBICglasso")
net_pred <- estimateNetwork(pooled_cor_pred, default = "EBICglasso")

# Network Comparison Test
nct_result <- NCT(net_gen, net_pred,
...
- Example: Generative AI network is more densely connected than predictive AI

**Network Structure:**
- p < .05 → Networks have different edge patterns
- Example: Generative AI has strong TRU—BI edge, predictive AI does not

**Edge-Specific:**
- p < .05 for specific edge → That edge differs between networks
- Example: TRU—BI edge is significantly stronger in generative AI network
...
layout <- averageLayout(net_gen, net_pred)

# Plot generative AI network
plot(net_gen,
     layout = layout,
     title = "Generative AI Network",
     theme = "colorblind")

# Plot predictive AI network
plot(net_pred,
     layout = layout,
     title = "Predictive AI Network",
     theme = "colorblind")
```
...
Centrality indices (strength, betweenness, expected influence) were computed to identify key constructs in the network. Stability was assessed via case-dropping bootstrap (1,000 iterations), with CS-coefficient > 0.50 indicating excellent stability. Edge weight accuracy was evaluated via nonparametric bootstrap (1,000 iterations) with 95% confidence intervals. Network comparison tests (NCT) evaluated whether network structure differed between generative and predictive AI subgroups."
...
"Network comparison tests revealed significant differences between generative AI and predictive AI networks (network structure invariance test: p = .012). The TRU—BI edge was significantly stronger in the generative AI network (ρ=.35) compared to predictive AI (ρ=.18, p = .008), supporting the hypothesis that trust is more critical for opaque generative systems. Conversely, the PE—BI edge was stronger in predictive AI (ρ=.40 vs. .28, p = .042), suggesting performance expectancy dominates for decision-support AI."
