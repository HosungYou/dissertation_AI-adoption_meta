...
This protocol describes the Bayesian extension to MASEM that uses Sabherwal et al. (2006) general IT meta-analysis as informative priors. This approach directly quantifies how AI adoption differs from general IT adoption, providing a unique academic contribution.
...
**Research Question:** How does AI adoption differ from general IT adoption?

**Traditional Approach (Frequentist MASEM):**
- Estimate AI adoption paths
- Informally compare to published IT meta-analyses
- Descriptive comparison only (no statistical test of difference)

**Bayesian Approach:**
- Use IT meta-analysis (Sabherwal et al., 2006) as **informative priors**
- Estimate AI adoption paths given IT prior knowledge
- **Quantify difference:** How much do AI paths deviate from IT priors?
- **Statistical evidence:** Bayes Factors test "AI ≠ IT" hypothesis

---

### 1.2 Why Sabherwal et al. (2006)?
...
- General IT adoption (not AI-specific)
...
**Applicability:**
- TAM/UTAUT constructs map directly to our constructs
- Provides effect size estimates and standard errors
- Ideal as informative priors for AI adoption paths

---

## 2. Prior Specification

### 2.1 Informative Priors (from Sabherwal et al., 2006)

**Core TAM/UTAUT Paths:**

| Path | Our Constructs | Sabherwal Constructs | Prior Mean (r) | Prior SD | Prior Distribution |
|------|----------------|----------------------|----------------|----------|-------------------|
| PE → BI | Performance Expectancy → Behavioral Intention | Perceived Usefulness → Intention | 0.52 | 0.05 | N(0.52, 0.05) |
| EE → BI | Effort Expectancy → Behavioral Intention | PEOU → Intention | 0.38 | 0.06 | N(0.38, 0.06) |
| SI → BI | Social Influence → Behavioral Intention | Subjective Norm → Intention | 0.34 | 0.05 | N(0.34, 0.05) |
| FC → UB | Facilitating Conditions → Use Behavior | Facilitating Conditions → Use | 0.25 | 0.08 | N(0.25, 0.08) |
| ATT → BI | Attitude → Behavioral Intention | Attitude → Intention | 0.40 | 0.06 | N(0.40, 0.06) |
| BI → UB | Behavioral Intention → Use Behavior | Intention → Use | 0.48 | 0.07 | N(0.48, 0.07) |

**Notes:**
- Means are meta-analytic correlations from Sabherwal et al. (2006)
- SDs estimated from heterogeneity statistics (I², confidence intervals)
- If Sabherwal reported β instead of r, convert using Peterson & Brown (2005) or use reported correlation when available
...
**Step 2: Specify Bayesian SEM with Priors**

```r
library(blavaan)

# Model 1 specification (TAM/UTAUT Core)
model1_bayes <- '
  # Structural paths with informative priors from Sabherwal et al. (2006)
  BI ~ prior("normal(0.52, 0.05)")*PE +     # Sabherwal PU→BI
       prior("normal(0.38, 0.06)")*EE +     # Sabherwal PEOU→BI
       prior("normal(0.34, 0.05)")*SI +     # Sabherwal SN→BI
       prior("normal(0.40, 0.06)")*ATT      # Sabherwal ATT→BI

  UB ~ prior("normal(0.48, 0.07)")*BI +     # Sabherwal BI→Use
       prior("normal(0.25, 0.08)")*FC       # Sabherwal FC→Use

  ATT ~ prior("normal(0.50, 0.07)")*PE +    # TAM path (estimated from literature)
        prior("normal(0.45, 0.07)")*EE      # TAM path
...
```r
model2_bayes <- '
  # TAM/UTAUT paths (informative priors)
  ...
  ATT ~ prior("normal(0.50, 0.07)")*PE +
        prior("normal(0.45, 0.07)")*EE
...
**Research Question:** Do AI paths differ from IT priors (Sabherwal et al., 2006)?

**Approach:** Examine how much posterior mean deviates from prior mean

**Example: PE → BI Path**

- **IT Prior (Sabherwal):** r = 0.52, SD = 0.05
- **AI Posterior:** r = 0.48, SD = 0.03
- **Difference:** 0.48 - 0.52 = -0.04 (AI lower than IT)
- **95% HPD:** [0.42, 0.54] (includes prior mean 0.52)

**Interpretation:**
- AI performance expectancy → intention path is slightly weaker than general IT
- But difference is small and 95% credible interval overlaps with IT prior
- **Conclusion:** AI and IT are similar for PE→BI path

---

**Example: TRU → BI Path (AI-Specific)**

- **IT Prior:** None (AI-specific construct)
- **Weakly Informative Prior:** r = 0.00, SD = 0.20
- **AI Posterior:** r = 0.32, SD = 0.05
- **95% HPD:** [0.22, 0.42]

**Interpretation:**
- Trust is a significant predictor of AI adoption (posterior far from prior mean of 0)
- Data strongly updates prior (posterior SD much smaller than prior SD)
- **Conclusion:** Trust is critical for AI (not captured in traditional IT models)
...
**Hypotheses:**
- H₀: β_AI = β_IT_prior (no difference)
- H₁: β_AI ≠ β_IT_prior (AI differs from IT)

**Bayes Factor (BF₁₀):**
- BF₁₀ > 10: Strong evidence for H₁ (AI ≠ IT)
- BF₁₀ = 3-10: Moderate evidence for H₁
- BF₁₀ = 1: No evidence (data equally likely under both hypotheses)
- BF₁₀ < 1: Evidence for H₀ (AI = IT)
...
|------|------|----------------|
| PE → BI | 2.1 | Weak evidence AI ≠ IT (posterior similar to prior) |
| EE → BI | 1.3 | No evidence of difference |
| TRU → BI | 85.2 | Very strong evidence that trust matters for AI (posterior far from 0) |
| ANX → BI | 12.5 | Strong evidence that anxiety reduces AI adoption |
...
"To directly quantify how AI adoption differs from general IT adoption, we employed Bayesian MASEM using informative priors derived from Sabherwal et al. (2006), a comprehensive meta-analysis of IT adoption across 121 studies (N > 50,000). For traditional TAM/UTAUT paths (PE→BI, EE→BI, SI→BI, FC→UB, ATT→BI, BI→UB), we specified normal priors centered on Sabherwal et al.'s meta-analytic correlations with standard deviations based on heterogeneity estimates. For AI-specific paths (TRU→BI, ANX→BI, TRA→TRU, AUT→ANX), we used weakly informative priors (M=0, SD=0.20) to allow data to dominate while regularizing extreme estimates.
...
| Path | IT Prior (Sabherwal) | AI Posterior Mean | 95% HPD | BF₁₀ | Interpretation |
|------|---------------------|-------------------|---------|------|----------------|
| PE → BI | 0.52 [0.42, 0.62] | 0.48 [0.42, 0.54] | [0.42, 0.54] | 2.1 | AI similar to IT |
| EE → BI | 0.38 [0.26, 0.50] | 0.35 [0.27, 0.43] | [0.27, 0.43] | 1.3 | AI similar to IT |
| TRU → BI | 0.00 [-.40, .40]* | 0.32 [0.22, 0.42] | [0.22, 0.42] | 85.2 | Strong AI-specific effect |
| ANX → BI | 0.00 [-.40, .40]* | -0.18 [-.24, -.12] | [-.24, -.12] | 12.5 | Strong negative effect for AI |

*Weakly informative prior (no IT equivalent)

---

**Narrative:**

"Bayesian MASEM using IT adoption priors revealed that traditional TAM/UTAUT paths were largely similar for AI and general IT. The performance expectancy → intention path showed a posterior mean of 0.48 (95% HPD: [0.42, 0.54]), only slightly lower than the IT prior of 0.52, with weak evidence of difference (BF₁₀ = 2.1). Similarly, effort expectancy → intention (posterior M = 0.35) closely matched the IT prior (M = 0.38), BF₁₀ = 1.3.

In contrast, AI-specific constructs demonstrated strong effects not captured in traditional IT models. Trust in AI emerged as a substantial predictor of adoption intention (β = 0.32, 95% HPD: [0.22, 0.42], BF₁₀ = 85.2), with very strong evidence supporting its inclusion. AI anxiety showed a negative effect on intention (β = -0.18, 95% HPD: [-.24, -.12], BF₁₀ = 12.5). These findings indicate that while AI adoption follows general technology acceptance patterns, AI-specific constructs provide critical additional explanatory power."
...
**Quantifies AI vs. IT Differences:**
- First study to statistically test whether AI adoption differs from IT adoption
- Uses rigorous Bayesian framework (not just descriptive comparison)
- Provides Bayes Factors as evidence metrics

**Leverages Existing Knowledge:**
- Builds on 20+ years of IT meta-analytic research (Sabherwal et al., 2006)
- Shows continuity and discontinuity with IT literature
- Updates IT priors with AI data

**Methodological Innovation:**
- Demonstrates Bayesian MASEM in technology adoption context
- Provides template for future "Technology X vs. general IT" comparisons
- Shows how to incorporate meta-analytic priors

---

### 9.2 Theoretical Implications

**If AI ≈ IT (posteriors close to priors):**
- AI is "just another technology"
- Existing TAM/UTAUT theory generalizes
- Organizations can apply known IT adoption strategies

**If AI ≠ IT (posteriors deviate from priors):**
- AI is qualitatively different from general IT
- Requires AI-specific theory (trust, anxiety, transparency)
- Organizations need new strategies for AI adoption

**Expected Result (based on literature):**
- Traditional paths similar (AI ≈ IT for PE, EE, SI)
- AI-specific paths add substantial value (TRU, ANX unique to AI)
- **Conclusion:** AI is both continuous (follows TAM/UTAUT) and discontinuous (requires AI extensions)
