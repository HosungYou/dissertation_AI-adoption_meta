...
- Predictive AI (recommendations, analytics)
- Other/Mixed

**Rationale:** Different AI types may have different reporting conventions.

**3. Geographic Region (4 strata):**
- North America
- Europe
- Asia
- Other (South America, Africa, Oceania, Multi-region)

**Rationale:** Cultural differences may affect construct labeling and measurement.
...
studies <- studies %>%
  mutate(
    ...
    ai_type_stratum = ai_type,  # Already coded
...
strata_props <- studies %>%
  group_by(year_stratum, ai_type_stratum, region_stratum) %>%
  summarize(n = n()) %>%
...
- 5 pilot studies with diverse characteristics:
  - 1 TAM study with clear labeling
  - 1 UTAUT study with standard constructs
  - 1 AI-specific study with trust/anxiety
  - 1 study with only β coefficients
  - 1 study with ambiguous construct labels
...
- Graduate student in information systems, psychology, or related field
- Familiarity with TAM/UTAUT literature
- Experience reading empirical research articles
...
- Familiarity with AI adoption research
...
```r
# AI Type
ai_type_kappa <- kappa2(merged[, c("ai_type_a", "ai_type_b")])

# Industry
industry_kappa <- kappa2(merged[, c("industry_a", "industry_b")])

print(paste("AI Type κ:", round(ai_type_kappa$value, 3)))
print(paste("Industry κ:", round(industry_kappa$value, 3)))
```
...
|---------------|--------|-------|--------|----------------|
| Correlations (r) | ICC(2,1) | .97 | [.95, .98] | Excellent |
| Correlations (r) | MAE | .018 | — | Excellent |
| Construct Mapping | κ | .92 | [.88, .96] | Almost Perfect |
| AI Type | κ | .88 | [.80, .96] | Almost Perfect |
| Industry Sector | κ | .85 | [.76, .94] | Almost Perfect |
| Reliabilities (α) | ICC(2,1) | .99 | [.98, .99] | Excellent |
...
## 10. Reporting ICR in Dissertation

### 10.1 Methods Section

**Template:**

"To ensure coding reliability, two independent coders extracted data from a stratified random sample of 30 studies (20% of total included studies). Stratification was based on publication year, AI type, and geographic region to ensure representativeness. Coders underwent 9 hours of training including orientation, pilot coding, and calibration. Inter-coder reliability was assessed using Cohen's kappa for categorical variables (construct mappings, moderators) and intraclass correlation coefficients (ICC) for continuous variables (correlation coefficients). Reliability was excellent: ICC(2,1) = .97 [95% CI: .95, .98] for correlations with mean absolute error (MAE) = .018; κ = .92 [.88, .96] for construct mappings. All metrics exceeded pre-specified thresholds (ICC ≥ .95, κ ≥ .85, MAE ≤ .03). Discrepancies were resolved through consensus discussion and reference to original study texts. The remaining 70% of studies were coded by a single trained coder with 10% random spot-checks."
