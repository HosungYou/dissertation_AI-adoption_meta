...
| `country` | text | Country/region of data collection | ISO 3166-1 alpha-2 codes | Use 2-letter codes (US, CN, DE). For multi-country: "XX" (mixed). |
| `ai_type` | cat | Type of AI technology studied | `generative`, `predictive`, `conversational`, `robotic`, `mixed`, `general` | **generative**: ChatGPT, DALL-E, etc. **predictive**: ML algorithms, recommender systems. **conversational**: chatbots, voice assistants. **robotic**: physical robots. **mixed**: 2+ categories. **general**: unspecified AI. |
| `domain` | cat | Application domain | `education`, `healthcare`, `business`, `personal`, `creative`, `other` | Context where AI is used. |
...
| `sample_type` | cat | Sample composition | `students`, `workers`, `general_public`, `mixed` | **students**: university/school samples. **workers**: employees in organizations. **general_public**: recruited from population. **mixed**: 2+ categories. |
| `mandatory_use` | cat | Voluntariness of AI use | `voluntary`, `mandatory`, `mixed`, `unclear` | **voluntary**: users choose to adopt. **mandatory**: required by employer/institution. **unclear**: not specified. |
