...
**Common Issues:**
- **Student samples:** Not inherently high risk, but limited generalizability if extrapolating to workforce/general population. Rate as "Some Concerns" unless study claims generalizability beyond students.
- **Snowball sampling:** Typically "Some Concerns" due to non-random selection, unless very large and diverse.
- **Online panels (MTurk, Prolific):** "Some Concerns" — convenience but diverse if attention checks and quality controls used.

**MASEM-Specific Note:** Heterogeneous sampling across studies is desirable for meta-analysis (increases generalizability). Selection bias is study-level concern; meta-analysis can estimate population effect if most studies are low/moderate risk.
